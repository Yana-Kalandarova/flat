...
const init = async () => {
	...
	fs.writeFileSync(`../src/data/responses/${currDay}_${currMonth}_${currYear}.json`, responseStr, (err) => {
		console.log(err);
	});

	const apartmentsList = fs.readFileSync('../src/data/apartments.json');
	...
	apartmentsArr.forEach(item => {
		...
		const path = `../src/data/apartments/${id}.json`;
		...
		fs.writeFileSync('../src/data/apartments.json', JSON.stringify(apartmentsListData, null, 2));
	});
};
